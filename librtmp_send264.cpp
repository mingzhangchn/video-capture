...
{    
    nalu.size = 40960;
	if (frame_get((uint8_t*)nalu.data, (uint32_t*)&nalu.size) == 0){
...
{    
    ...
    uint8_t tBuf[2048] = {0};
    int tBufLen = 2048;
    frame_get(tBuf, (uint32_t*)&tBufLen);
	metaData.nSpsLen = tBufLen;  
	metaData.Sps=NULL;
	metaData.Sps=(unsigned char*)malloc(metaData.nSpsLen);
	memcpy(metaData.Sps,tBuf,metaData.nSpsLen);
...
got_sps_pps:
		...
		tick +=tick_gap;
		now=RTMP_GetTime();
        int waitTime = now - last_update > tick_gap ? 0 : tick_gap - now + last_update;
